...
import { cancelAiRun } from "../lib/ipc/commands";
import { onAiOutput, type AiOutputEvent } from "../lib/ipc/events";
...
/** The backend's marker for a run it cut short itself (`AiRunRegistry.TimedOutMarker`). */
export const TIMED_OUT_MARKER = "RUN_TIMED_OUT::";

/**
 * True when the run was ended by its own deadline rather than by the user or by a failure.
 *
 * Kept apart from `isCancellation` because the two say opposite things to the person reading the
 * panel: one is "you stopped this", the other is "this never finished on its own".
 */
export function isTimeout(error: unknown): boolean {
  return String(error).includes(TIMED_OUT_MARKER);
}

/** The deadline, in whole minutes, as the backend reported it — or `null` if it did not say. */
export function timeoutMinutes(message: string): string | null {
  const index = message.indexOf(TIMED_OUT_MARKER);
  if (index < 0) return null;
  const minutes = message.slice(index + TIMED_OUT_MARKER.length).trim();
  return /^\d+$/.test(minutes) ? minutes : null;
}
