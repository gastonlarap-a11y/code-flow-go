...
import { deleteWorkspaceMcp, listWorkspaceMcps, upsertWorkspaceMcp } from "../../lib/ipc/commands";
...
export function McpSettings() {
  ...
  if (!workspaceId) {
    return (
      <section>
        <h3 className="mb-1 text-title font-semibold">{t("settings.mcpsTitle")}</h3>
        <p className="text-relaxed text-[var(--cf-text-muted)]">{t("settings.mcpsSelectWorkspace")}</p>
      </section>
    );
  }
  ...
  return (
    <section>
      <div className="mb-1 flex items-center justify-between">
        <h3 className="text-title font-semibold">{t("settings.mcpsTitle")}</h3>
        <Button variant="ghost" size="sm" icon={Plus} onClick={addMcp}>
          {t("settings.addMcp")}
        </Button>
      </div>
      <p className="mb-3 text-relaxed text-[var(--cf-text-muted)]">{t("settings.mcpsHint")}</p>

      {!agentic && (
        <p className="mb-3 rounded-md border border-[var(--cf-border)] bg-[color-mix(in_oklab,var(--cf-warning)_10%,transparent)] px-2.5 py-1.5 text-body text-[var(--cf-text-muted)]">
      ...
      <div className="space-y-2">
            ...
            <div key={mcp.id} className="rounded-lg border border-[var(--cf-border)]">
              <div className="flex items-center gap-2 p-2.5">
                ...
                >
                  ...
                  />
                  <span className={`truncate text-body font-medium ${mcp.enabled ? "" : "text-[var(--cf-text-muted)]"}`}>
                    {mcp.name || t("settings.newMcpName")}
                  </span>
                  {mcp.command && !isOpen && (
                    <span className="truncate font-mono text-badge text-[var(--cf-text-muted)]">{mcp.command}</span>
                  )}
                  {!mcp.enabled && (
                    <span className="shrink-0 rounded-full bg-black/[0.05] px-1.5 py-0.5 text-badge text-[var(--cf-text-muted)] dark:bg-white/[0.08]">
                      {t("settings.disabled")}
                    </span>
                  )}
                </button>
                ...
                {/* One row among many: a labelled 24px icon rather than a wide button, but the
                    label is required either way. It had none at all. */}
                <IconButton
                  label="settings.removeMcp"
                  icon={Trash2}
                  variant="danger"
                  className="shrink-0"
                  onClick={() => remove(mcp)}
              ...
              {isOpen && (
                <div className="border-t border-[var(--cf-border)] p-3">
                  <input
                    value={mcp.name}
                    aria-label={t("settings.newMcpName")}
                    onChange={(e) => update(mcp, { name: e.target.value })}
                    placeholder={t("settings.newMcpName")}
                    className="mb-1.5 w-full rounded-md border border-[var(--cf-border)] bg-transparent px-2 py-1 text-body font-medium outline-none focus:border-[var(--cf-accent)]"
                  />
                  ...
                  <textarea
                    value={mcp.env}
                    aria-label={t("settings.mcpEnv")}
                    onChange={(e) => update(mcp, { env: e.target.value })}
                    rows={2}
                    placeholder={t("settings.mcpEnvPlaceholder")}
                    className="w-full resize-none rounded-md border border-[var(--cf-border)] bg-transparent px-2 py-1 font-mono text-body outline-none focus:border-[var(--cf-accent)]"
                  />
                </div>
              )}
            </div>
        ...
        {mcps.length === 0 && <p className="text-body text-[var(--cf-text-muted)]">{t("settings.noMcps")}</p>}
      </div>
    </section>
  );
}
