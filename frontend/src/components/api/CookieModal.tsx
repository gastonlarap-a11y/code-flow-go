...
export function CookieModal({ onClose }: { onClose: () => void }) {
  ...
  const upsertCookie = useApiCookieStore((s) => s.upsertCookie);
  const deleteCookie = useApiCookieStore((s) => s.deleteCookie);
  ...
  return (
    <ApiModal
      ...
      footer={
        <>
          <Button variant="ghost" size="sm" icon={Plus} onClick={addPending} disabled={workspaceId === null}>
            {t("api.cookie.add")}
          </Button>
    ...
    >
      <div className="min-h-0 flex-1 overflow-auto p-3">
        {cookies.length === 0 && pending.length === 0 && (
          <p className="p-3 text-ui text-[var(--cf-text-muted)]">{t("api.cookie.none")}</p>
        )}

        {domains.map((domain) => (
          <section key={domain} className="mb-4">
            <h3 className="mb-1 font-mono text-badge font-semibold text-[var(--cf-accent)]">
              {domain}
            </h3>
              ...
              return (
                ...
                >
                  ...
                  <Field
                    ...
                    ariaLabel={t("api.cookie.expires")}
