import { getSetting, hasAdoPat, setSetting } from "./ipc/commands";
...
export async function loadAdoConnections(): Promise<AdoConnection[]> {
  ...
  // Existence is the whole question here — the PAT's value was never used, only its presence.
  // Asking `has` instead of `get` is what lets the sidecar keep the secret (DIVERGENCE-SEC-d).
  const legacyOrg = await getSetting(LEGACY_ORG_KEY);
  if (legacyOrg) {
    const saved = await hasAdoPat(legacyOrg).catch(() => false);
    if (saved) return [{ org: legacyOrg }];
  }
  return [];
}
