...
import { apiSendHttp, apiSendHttpTracked } from "../ipc/apiCommands";
...
export function buildImplicitHeaders(req: ResolvedRequest): [string, string][] {
  ...
  const candidates: [string, string][] = [
    ...
    ["User-Agent", "CodeFlow"],
    ["Accept", "*/*"],
    // Must mirror `ADVERTISED_ENCODINGS` in `HttpSend.cs`.
...
function graphqlPayload(spec: ApiRequestSpec, ctx: VariableContext): string {
  ...
  if (rawVariables) {
    try {
      ...
      throw new Error(`GraphQL variables are not valid JSON: ${String(e)}`, { cause: e });
    }
  }
