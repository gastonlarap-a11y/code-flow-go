...
import { monaco } from "../../lib/monacoEditor";
...
export function GraphqlPanel({ tabId }: { tabId: string }) {
  const t = useT();
  const graphql = useApiTabsStore((s) => s.openTabs.find((tab) => tab.id === tabId)?.draft.body.graphql ?? null,
  );
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex shrink-0 items-center gap-2 border-b border-[var(--cf-border)] px-3 py-1.5">
          <input
            ...
            className="w-48 rounded-md border border-[var(--cf-border)] bg-[var(--cf-surface)] px-2 py-0.5 text-ui text-[var(--cf-text)] outline-none placeholder:text-[var(--cf-text-muted)] focus:border-[var(--cf-accent)]"
          />
          <Button
            variant="secondary"
            size="sm"
          ...
          <Tooltip label={status}>
            <span
              className={`min-w-0 flex-1 truncate text-badge ${
          ...
          <IconButton
            label="api.graphql.explorer"
            icon={explorerOpen ? PanelRightClose : PanelRightOpen}
            active={explorerOpen}
            className="shrink-0"
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          <div className="flex shrink-0 items-center gap-1 px-3 pt-1.5 text-badge font-medium uppercase tracking-wide text-[var(--cf-text-muted)]">
            {t("api.graphql.query")}
          </div>
    ...
    <>
      <div className="flex shrink-0 items-center gap-1.5 border-b border-[var(--cf-border)] px-2 py-1.5">
        ...
        <input
          ...
          className="min-w-0 flex-1 bg-transparent text-ui text-[var(--cf-text)] outline-none placeholder:text-[var(--cf-text-muted)]"
        />
      </div>

      <div className="min-h-0 flex-1 overflow-auto px-1 py-1.5">
        {roots.length === 0 && types.length === 0 && (
          <p className="px-2 py-4 text-ui text-[var(--cf-text-muted)]">{t("api.graphql.noMatches")}</p>
        )}

        {roots.map((root) => (
          <div key={root.operation} className="mb-2">
            <div className="px-2 pb-0.5 text-badge font-semibold uppercase tracking-wide text-[var(--cf-text-muted)]">
              {root.name}
            </div>
            {root.fields.map((field) => (
              <Tooltip key={field.name} label={t("api.graphql.insertField")}>
              <button
                onClick={() => onInsert(root.operation, field)}
                className="flex w-full items-baseline gap-1.5 rounded px-2 py-1 text-left hover:bg-[var(--cf-accent-soft)]"
              >
                <span className="truncate text-ui text-[var(--cf-text)]">{field.name}</span>
        ...
        {types.length > 0 && (
          <div className="px-2 pb-0.5 pt-1 text-badge font-semibold uppercase tracking-wide text-[var(--cf-text-muted)]">
            {t("api.graphql.types")}
          </div>
        )}
        {types.map((type) => (
...
function TypeRow({ type, open, onToggle }: { type: GqlType; open: boolean; onToggle: () => void }) {
  return (
    <div>
      <button
        onClick={onToggle}
        className="flex w-full items-center gap-1 rounded px-1 py-1 text-left hover:bg-black/[0.04] dark:hover:bg-white/[0.06]"
      >
        {open ? (
        ...
        <span className="truncate text-ui text-[var(--cf-text)]">{type.name}</span>
        <span className="ml-auto shrink-0 text-badge uppercase text-[var(--cf-text-muted)]">
          {type.kind.replace("_", " ")}
        </span>
      </button>
      ...
      {open && (
        <div className="mb-1 ml-3 border-l border-[var(--cf-border)] pl-2">
          {type.description && (
            <p className="py-0.5 text-badge leading-snug text-[var(--cf-text-muted)]">{type.description}</p>
          )}
  ...
  return (
    <div className="py-0.5">
      <div className="flex items-baseline gap-1.5">
        <span className="truncate text-ui text-[var(--cf-text)]">{name}</span>
        {type && <span className="truncate text-badge text-[var(--cf-text-muted)]">{type}</span>}
      </div>
      {description && (
        <p className="text-badge leading-snug text-[var(--cf-text-muted)]">{description}</p>
      )}
    </div>
  );
}
