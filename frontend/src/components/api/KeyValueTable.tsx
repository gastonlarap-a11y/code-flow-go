...
const CELL =
  "w-full rounded bg-transparent px-2 py-1.5 text-ui leading-5 outline-none placeholder:text-[var(--cf-text-muted)]";
    ...
    return (
      <div className="flex min-h-0 flex-col gap-2">
        <div className="flex justify-end">
          <Button variant="ghost" size="sm" onClick={() => setBulk(false)}>
            {t("api.keyValueEdit")}
          </Button>
        </div>
        <textarea
          ...
          className="min-h-[160px] w-full resize-y rounded-md border border-[var(--cf-border)] bg-transparent p-2 font-mono text-ui leading-5 text-[var(--cf-text)] outline-none focus:border-[var(--cf-accent)]"
        />
      </div>
    );
  }
    ...
    <div className="flex min-w-0 flex-col">
      ...
      <div className="min-w-0 overflow-hidden rounded-md border border-[var(--cf-border)]">
        <div
          ...
          className="grid items-center border-b border-[var(--cf-border)] bg-[var(--cf-surface)] px-1 py-1 text-badge font-semibold uppercase tracking-wide text-[var(--cf-text-muted)]"
        >
          <span />
        ...
        {displayed.map(({ row, isDraft }) => {
          ...
          return (
            ...
            >
              ...
              {readOnlyKeys ? (
                <span className="truncate px-2 py-1.5 text-ui leading-5 text-[var(--cf-text-muted)]">
                  {row.key}
                </span>
              ) : (
                <VariableInput
              ...
              {readOnlyKeys ? (
                ...
                <Tooltip label={row.src || t("api.body.chooseFile")}>
                <button
                  onClick={() => void pickFile(row, isDraft)}
                  className="mx-1 flex min-w-0 items-center gap-1.5 rounded px-1.5 py-1 text-ui text-[var(--cf-text)] hover:bg-[var(--cf-accent-soft)]"
                >
              ...
              {readOnlyKeys ? (
                <span className="px-2 py-1.5 text-ui leading-5 text-[var(--cf-text-muted)]" />
              ) : (
                <input
                  type="text"
