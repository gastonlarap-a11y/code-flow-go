  ...
  return (
    <div className="flex items-start justify-between gap-3 border-b border-[var(--cf-border)] py-2 last:border-b-0">
      <div className="min-w-0">
        <p className="text-ui text-[var(--cf-text)]">{label}</p>
...
export function RequestSettingsPanel({ tabId }: { tabId: string }) {
  ...
  const globals = useApiSettingsStore((s) => s.settings);
    ...
    return (
      ...
      >
        <input
          ...
          className="w-24 rounded-md border border-[var(--cf-border)] bg-[var(--cf-surface)] px-2 py-1 text-right font-mono text-ui text-[var(--cf-text)] outline-none focus:border-[var(--cf-accent)] placeholder:text-[var(--cf-text-muted)]"
        />
      </SettingRow>
    );
  };
    ...
    <div className="flex h-full min-h-0 flex-col overflow-auto p-3">
      <div className="max-w-[620px]">
        ...
        <SettingRow
          ...
          overridden={false}
          note={
            <p className="mt-1 flex items-start gap-1.5 text-badge text-[var(--cf-warning)]">
