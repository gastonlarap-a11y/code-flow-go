          ...
          return (
            <Fragment key={tab.path}>
              ...
              <div
                ...
                // Named explicitly: a `role="tab"` composes its name from its contents, and the
                // close button inside it is content — the tab announced itself as "app.tsx Close".
                aria-label={tab.path}
                ...
                className={`group relative flex h-9 max-w-[220px] shrink-0 cursor-pointer select-none items-center gap-1.5 border-r border-[var(--cf-border)] pl-3 pr-2 text-ui transition-colors ${
                  active
              ...
              >
                ...
                {suffix && (
                  <span className="truncate text-badge text-[var(--cf-text-muted)] opacity-70">{suffix}</span>
                )}
                {/* Unsaved state and "close this" are two different things, and merging them into
                    one 16px slot that appeared on hover left no way to close a tab by keyboard or
                    on a touch screen. Two marks now, both always there. */}
                {tab.dirty && (
                  <Tooltip label={t("editor.unsaved")}>
                    <span className="ml-auto h-2 w-2 shrink-0 rounded-full bg-[var(--cf-accent)]" />
                  </Tooltip>
                )}
                <IconButton
                  label="editor.closeTab"
                  icon={X}
      ...
      {draggingHere &&
        ...
        createPortal(
          <div
            ...
            className="pointer-events-none fixed left-0 top-0 z-[100] flex items-center gap-1.5 rounded-md border border-[var(--cf-accent)] bg-[var(--cf-surface)] px-2 py-1 text-badge text-[var(--cf-text)] shadow-lg"
          >
            {baseName(drag.path)}
          </div>,
          document.body,
