  ...
  rows.forEach(({ commit, row, lane }) => {
    commit.parent_ids.forEach((parentId) => {
      ...
      edges.push({
        ...
        fromLane: lane,
        toRow: parentRow,
        // `rows` was fully populated by the first `commits.forEach` above, at every index up to
        // `commits.length - 1`, which is the same range `parentRow` (from `indexById`) ranges over.
...
export function laneColor(lane: number): string {
  // LANE_COLORS is a non-empty const array, so `lane % LANE_COLORS.length` is always a valid index.
