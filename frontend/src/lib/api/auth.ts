...
function parseJsonObject(text: string, what: string): Record<string, unknown> {
  ...
  try {
    ...
    throw new Error(`The JWT ${what} is not valid JSON: ${describe(error)}`, { cause: error });
  }
 ...
 *
 * Goes through the sidecar transport rather than `fetch` — the webview's fetch is subject to CORS
...
function decodeBase64(text: string, what: string): Uint8Array<ArrayBuffer> {
