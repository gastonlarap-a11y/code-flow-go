...
export interface AiProviderOption {
   ...
   * engine's `default_binary()` in the sidecar. For Ollama this is the HTTP endpoint. */
...
export const AI_PROVIDERS: AiProviderOption[] = [
  ...
  // `gemini` CLI, against a Google-account login. Headless via `agy -p`. See the gemini engine adapter.
  {
    id: "gemini",
    label: "Gemini",
    icon: Gem,
  ...
  // `codex exec`. See the codex engine adapter.
  {
    id: "codex",
    label: "Codex",
    icon: Cpu,
  ...
  // `opencode run`. See the opencode engine adapter.
  {
    id: "opencode",
    label: "Open Code",
    icon: SquareTerminal,
  ...
  // are hidden when it's active. `defaultBinary` is the endpoint. See the ollama engine adapter.
  {
    id: "ollama",
    label: "Ollama",
    icon: HardDrive,
  ...
  // with an API key from the OS keyring; non-agentic (no tool loop). See the openai engine adapter.
  {
    id: "openai",
    label: "OpenAI",
    icon: Sparkles,
