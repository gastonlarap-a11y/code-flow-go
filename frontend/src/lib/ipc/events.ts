import { listen } from "../bridge/host";
// Re-exported so components can type an unsubscribe handle without reaching into lib/bridge.
export type { UnlistenFn } from "../bridge/host";
...
/**
 * Whether the .NET core is answering.
 *
 * The shell has emitted this since the sidecar became a separate process and nothing listened to it,
 * which is how a core that failed to start looked like an app whose buttons did nothing. Pair it with
 * `host.sidecarStatus()` rather than using it alone: a core that never spawns is `down` before this
 * window has attached a handler, so the event on its own can only report a late failure.
 */
export const onSidecarStatus = (handler: (event: SidecarStatusEvent) => void) =>
  listen<SidecarStatusEvent>("codeflow:sidecar-status", (e) => handler(e.payload));
