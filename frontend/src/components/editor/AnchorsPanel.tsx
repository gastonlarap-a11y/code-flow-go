...
import { searchRepo } from "../../lib/ipc/commands";
    ...
    <div className="flex h-full min-h-0 flex-col">
      <div className="shrink-0 space-y-1.5 border-b border-[var(--cf-border)] p-1.5">
        <div className="flex items-center gap-1">
          <div className="flex items-center gap-0.5 rounded-md border border-[var(--cf-border)] p-0.5">
            {(
              [
                { id: "file", icon: FileText, labelKey: "anchors.scopeFile" },
                { id: "project", icon: FolderTree, labelKey: "anchors.scopeProject" },
              ] as const
        ...
        <div className="flex flex-wrap gap-1">
          {ANCHOR_TAGS.map((tag) => {
            ...
            return (
              <Tooltip key={tag.id} label={t("anchors.toggleTag", { tag: tag.id })}>
              <button
                onClick={() => toggleTag(tag.id)}
                aria-pressed={on}
                className={`flex items-center gap-1 rounded px-1 py-0.5 text-badge font-medium tracking-wide ${
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        ...
        {!scanning && !error && visible.length === 0 && (
          <p className="px-3 py-2 text-badge text-[var(--cf-text-muted)]">
            ...
            <div key={path} className="pb-1">
              ...
              {scope === "project" && (
                <div className="flex items-center gap-1.5 px-2 py-0.5">
                  ...
                  <span className="shrink-0 text-badge text-[var(--cf-text-muted)]">{items.length}</span>
                </div>
              )}
              {items.map((anchor) => (
                <button
                ...
                >
                  <span
                    className="shrink-0 text-badge font-semibold tracking-wide"
                    style={{ color: anchorColor(anchor.tag) }}
                  >
                    {anchor.tag}
                  </span>
                  <span className="min-w-0 flex-1 truncate text-badge text-[var(--cf-text-muted)]">
                    {anchor.text || <span className="italic opacity-60">{t("anchors.bare")}</span>}
                  </span>
                  <span className="shrink-0 font-mono text-badge text-[var(--cf-text-muted)]">{anchor.line}</span>
                </button>
              ))}
            </div>
          );
