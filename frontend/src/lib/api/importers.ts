...
function parseCurlTokens(tokens: string[], warnings: string[]): ApiRequestSpec | null {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (token.startsWith("--")) {
      ...
      if (CURL_VALUE_FLAGS.has(name)) {
        ...
        applyFlag(name, consumable ? (tokens[++i] ?? null) : null);
      }
      continue;
    }
    ...
    if (token.startsWith("-") && token.length > 1) {
      ...
      for (let c = 0; c < cluster.length; c++) {
        ...
        const long = CURL_SHORT[short];
        if (!long) {
          warnings.push(`Ignored unsupported cURL option: -${short}`);
          continue;
        }
  ...
  const [firstUrl] = urls;
  if (firstUrl === undefined) return null;
  if (urls.length > 1) {
    warnings.push(`Only the first of ${urls.length} URLs in the command was imported.`);
  }

  const split = splitQuery(normalizeUrl(firstUrl));
  spec.url = split.url;
