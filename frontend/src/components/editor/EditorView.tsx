...
import { Bug, FileCode, FileSearch, Files, Keyboard, Search, Tags, type LucideIcon } from "lucide-react";
...
import { readFileText, writeFileText } from "../../lib/ipc/commands";
...
/** Which panel the activity rail is showing. */
type SidePanel = "files" | "search" | "anchors" | "debug";

/** The activity rail's four panels, in the order they sit in the column. */
const RAIL_PANELS: { id: SidePanel; icon: LucideIcon; labelKey: TranslationKey }[] = [
  { id: "files", icon: Files, labelKey: "editor.explorer" },
  { id: "search", icon: Search, labelKey: "editor.searchInProject" },
  { id: "anchors", icon: Tags, labelKey: "anchors.title" },
  { id: "debug", icon: Bug, labelKey: "debug.title" },
];

/** The chord Monaco holds for a rail action, already formatted — or `undefined` if it holds none. */
function railChord(labelKey: TranslationKey): string | undefined {
...
export function EditorView() {
  ...
  const closeTab = useCallback(async (groupId: string, path: string) => {
    ...
    setGroups(next);
    // `closeTabInGroups` only drops an emptied group when more than one remains, so `next` is
    // never empty here.
  ...
  return (
    // No header strip: the project and branch it used to repeat are already in the command header,
    ...
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex min-h-0 flex-1 gap-1.5">
        ...
        <div className={`flex w-9 shrink-0 flex-col items-center gap-1 py-1.5 ${CARD}`}>
          {RAIL_PANELS.map(({ id, icon, labelKey }) => (
            // The pill and the running-session dot are positioned against this span rather than the
            // button, so `IconButton` stays the plain primitive it is everywhere else.
            <span key={id} className="relative flex">
              ...
              <IconButton
                ...
                size="md"
                active={sidePanel === id}
                // Read, not typed: these chords belong to Monaco, and the rail used to carry them
                // as a hardcoded suffix that a rebind would have left lying.
                {...(railChord(labelKey) ? { shortcutLabel: railChord(labelKey)! } : {})}
                className="relative"
          ...
          <IconButton
            label="editor.goToFile"
            icon={FileSearch}
            size="md"
            {...(railChord("editor.goToFile") ? { shortcutLabel: railChord("editor.goToFile")! } : {})}
            className="mt-auto"
