...
import { ChevronRight, Download, Loader2, Save, Send } from "lucide-react";
...
import { apiCancelHttp, apiSaveFile } from "../../lib/ipc/apiCommands";
...
import { getAuthChainForTab, getVariableContext, useApiStore } from "../../state/apiStore";
...
export function RequestBuilder({ tabId }: { tabId: string }) {
  const t = useT();
  const tab = useApiTabsStore((s) => s.openTabs.find((item) => item.id === tabId) ?? null);
  ...
  // `getVariableContext()` builds a fresh object on every call, so it can never be a selector —
  ...
  const update = (patch: Partial<ApiRequestSpec>) => useApiTabsStore.getState().updateDraft(tabId, patch);
  ...
  const save = async () => {
    ...
    if (current.requestId === null && current.collectionId === null) {
      const first = useApiTreeStore.getState().collections[0];
  ...
  const saveToTarget = async () => {
    ...
    const saved = await useApiTabsStore.getState().saveTab(tabId, {
  ...
  const persistScopeWrites = async (before: SandboxScopes, after: SandboxScopes) => {
    ...
    if (changed(before.collection, after.collection)) {
      const collection = useApiTreeStore.getState().collections.find((item) => item.id === collectionId);
      if (collection) await useApiTreeStore.getState().updateCollection({ ...collection, variables: JSON.stringify(after.collection) });
    }
  };
  ...
  const persistCookies = async (cookies: ParsedCookie[]) => {
    ...
    for (const cookie of cookies) {
      const store = useApiCookieStore.getState();
  ...
  const recordHistory = async (response: ApiResponse, request: ResolvedRequest | null) => {
    const workspaceId = useApiStore.getState().workspaceId;
    if (!useApiSettingsStore.getState().settings.saveHistory || workspaceId === null) return;
    const current = useApiTabsStore.getState().openTabs.find((item) => item.id === tabId);
    if (!current) return;
    await useApiHistoryStore.getState().addHistory({
      id: newId("hist"),
      workspace_id: workspaceId,
  ...
  const runSend = async (download: boolean) => {
    ...
    const before: SandboxScopes = getVariableContext(current.collectionId);
    let scopes = before;
  ...
  // A count says strictly more than a dot, so a section that can be counted shows the number and
  // the rest fall back to "there is something in here".
  const panelOptions = PANEL_ORDER.map((id) => {
    const count = badgeCount(id);
    const labelKey: TranslationKey =
      id === "body" && protocol === "graphql" ? "api.tab.query" : PANEL_LABELS[id];
    return count !== null
      ? { id, labelKey, badge: count }
      : { id, labelKey, dot: badgeDot(id) };
  });
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="flex shrink-0 items-center gap-2 border-b border-[var(--cf-border)] px-3 py-1.5">
        <input
          ...
          onChange={(e) => useApiTabsStore.getState().renameTab(tabId, e.target.value)}
          className="min-w-0 max-w-[280px] flex-none rounded bg-transparent px-1 py-0.5 text-body font-medium text-[var(--cf-text)] outline-none placeholder:text-[var(--cf-text-muted)] focus:bg-[var(--cf-surface)]"
        />
        {crumbs.length > 0 && (
          <span className="flex min-w-0 items-center gap-1 truncate text-badge text-[var(--cf-text-muted)]">
        ...
        <Tooltip label={t("api.changeProtocol")}>
        ...
        {tab.dirty && (
          <span className="shrink-0 text-badge text-[var(--cf-text-muted)]">{t("api.unsaved")}</span>
        )}
        <div className="relative shrink-0">
          <button
            onClick={() => void save()}
            className="flex items-center gap-1.5 rounded-md border border-[var(--cf-border)] px-2.5 py-1 text-ui text-[var(--cf-text)] hover:border-[var(--cf-accent)] hover:text-[var(--cf-accent)]"
          >
            <Save size={13} />
            {t("api.save")}
          </button>
          ...
          {savePicker && (
            <div className="absolute right-0 top-full z-50 mt-1 w-[320px] rounded-md border border-[var(--cf-border)] bg-[var(--cf-surface-raised)] p-3 shadow-[var(--cf-shadow)]">
              <p className="mb-2 text-badge font-semibold uppercase tracking-wide text-[var(--cf-text-muted)]">
                {t("api.saveTo")}
              </p>
              {collections.length === 0 ? (
                <p className="text-ui text-[var(--cf-text-muted)]">{t("api.noCollections")}</p>
              ) : (
                <div className="flex flex-col gap-2">
                  <label className="flex flex-col gap-1">
                    <span className="text-badge text-[var(--cf-text-muted)]">{t("api.scope.collection")}</span>
                    <Select
                      size="sm"
                  ...
                  <label className="flex flex-col gap-1">
                    <span className="text-badge text-[var(--cf-text-muted)]">{t("api.folder")}</span>
                    <Select
                      size="sm"
              ...
              <div className="mt-3 flex justify-end gap-2">
                <Button variant="ghost" size="sm" onClick={() => setSavePicker(null)}>
                  {t("common.cancel")}
                </Button>
                <Button variant="primary" size="sm" disabled={collections.length === 0} onClick={() => void saveToTarget()}>
                  {t("api.save")}
                </Button>
              </div>
            </div>
      ...
      <div className="flex shrink-0 items-stretch gap-2 px-3 py-2">
        {protocol === "http" ? (
          ...
          <span
            className="flex shrink-0 items-center rounded-md border border-[var(--cf-border)] px-2.5 font-mono text-badge font-semibold"
        ...
        <VariableInput
          ...
          fieldClassName="px-2.5 py-1.5 text-ui"
          onPaste={(e) => {
        ...
        {/* Send and send-and-download are two actions, so they are two buttons. The caret used to
            open a menu holding exactly one item — the same "menu built for N tools, shipping with
            one" shape the workspace menu was deleted for — and it cost a ref, an open flag and a
            click-outside listener to hide an action a single button can just show. */}
        {!isStreaming && !isGrpc && (
          <div className="flex shrink-0 items-center gap-1">
            {sending ? (
              // Filled square, not `X`: this stops a request that is running, it does not dismiss
              // a surface. The icon dictionary keeps the two apart.
              <Button variant="secondary" icon={StopSquare} onClick={cancelSend}>
                {t("api.cancel")}
              </Button>
            ) : (
              <>
      ...
      {isStreaming ? (
        ...
        <>
          <div className="flex min-h-0 flex-1 flex-col">
            {/* Automatic activation, unlike the AI panel's strip: these panels are already built
                and switching to one costs nothing, which is exactly the condition the APG puts on
                letting selection follow focus. */}
            <Tabs
              options={panelOptions}
              activeId={panel}
              onSelect={setPanel}
              layoutId="cf-api-panel"
              label={t("api.requestSections")}
              className="cf-tab-strip shrink-0 overflow-x-auto border-y border-[var(--cf-border)] px-3 py-1"
            />
            ...
            <div className="min-h-0 flex-1" {...tabPanelProps("cf-api-panel", panel)}>
              {panel === "params" && (
                <div className="flex h-full flex-col gap-4 overflow-auto p-3">
                  <section className="flex flex-col gap-1.5">
                    <h3 className="text-badge font-semibold uppercase tracking-wide text-[var(--cf-text-muted)]">
                      {t("api.queryParams")}
                    </h3>
                    <KeyValueTable
                      rows={spec.params}
                  ...
                  {implicitHeaders.length > 0 && (
                    <div className="flex flex-col gap-1.5">
                      <Button
                        ...
                        tooltip={showImplicit ? t("api.hideHiddenHeaders") : t("api.showHiddenHeaders")}
                        aria-expanded={showImplicit}
                        className="self-start !text-[var(--cf-accent)]"
                      >
                        <ChevronRight
                          size={14}
                          aria-hidden
              ...
              {panel === "docs" && (
                <div className="h-full overflow-auto p-3">
                  <textarea
                    ...
                    className="min-h-[200px] w-full resize-y rounded-md border border-[var(--cf-border)] bg-transparent p-2 text-ui leading-5 text-[var(--cf-text)] outline-none focus:border-[var(--cf-accent)]"
                  />
                </div>
              )}
            </div>
