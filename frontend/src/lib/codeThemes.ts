...
export function findTheme(id: string, mode: "light" | "dark"): CodeTheme {
  ...
  // DARK_THEMES and LIGHT_THEMES are non-empty const arrays, so `pool` always has a first entry.
...
export function resolveTokenRule(tokenType: string, rules: TokenRule[]): TokenRule {
  // Callers always pass `tokenRulesFor(theme)`, whose first entry is the `""` catch-all — `rules`
 ...
 * replacing the user's chosen accent would make that setting look broken. `--cf-accent-soft` and
 * `--cf-surface-raised` are re-derived here, though, because both mix the accent *into the surface*
 * — and the surface is exactly what just changed. Written as `color-mix(…)` rather than a computed
 * hex so the accent stays a live reference: `accentStore.apply()` restamps `--cf-accent` on its own
 * schedule, and a hex baked here would freeze whichever accent happened to be set at theme-switch
 * time.
 */
export function applyThemeVars(theme: CodeTheme) {
  ...
  // The 4% tint from `index.css`, re-derived against *this* theme's raised surface. Each of the 21
  // keeps its own colour and gains the same trace of the accent.
  root.setProperty(
    "--cf-surface-raised",
    `color-mix(in oklch, ${theme.ui.surfaceRaised}, var(--cf-accent) 4%)`,
  );
