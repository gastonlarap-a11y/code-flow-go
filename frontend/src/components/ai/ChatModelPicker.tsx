...
export function ChatModelPicker({ liveModel, chatActive }: { liveModel: string | null; chatActive: boolean }) {
  ...
  // AI_PROVIDERS is a fixed, non-empty catalog — the fallback always resolves.
  ...
  return (
    <>
      <Tooltip label={t("chat.changeModelTitle")}>
      <button
        ref={triggerRef}
        onClick={() => (open ? setOpen(false) : openMenu())}
        aria-haspopup="menu"
        aria-expanded={open}
        className="cf-focusable flex h-6 max-w-full items-center gap-1 rounded-md border border-transparent px-1.5 text-badge text-[var(--cf-text-muted)] hover:border-[var(--cf-border)] hover:bg-black/[0.03] dark:hover:bg-white/[0.05]"
      >
      ...
      {open &&
        createPortal(
          ...
          >
            {browsing === null ? (
              <>
                <p className="shrink-0 px-2.5 py-1.5 text-badge font-medium uppercase tracking-wide text-[var(--cf-text-muted)]">
                  {t("chat.modelForChat")}
                </p>
                <div className="min-h-0 flex-1 overflow-auto p-1 pt-0">
                  {selectable.map((p) => {
                    ...
                    return (
                      // A disabled row fires no pointer events, so the reason it is locked has to
                      // be anchored on the wrapper rather than on the button.
                      <Tooltip key={p.id} label={locked ? t("chat.providerLocked") : ""}>
                      <button
                        onClick={() => browse(p.id)}
                        disabled={unavailable || locked}
                        className={`cf-focusable flex w-full items-center gap-1.5 rounded-md px-2 py-1.5 text-left text-ui disabled:opacity-40 ${
                          p.id === providerId
                      ...
                      >
                        ...
                        {unavailable ? (
                          <span className="shrink-0 text-badge text-[var(--cf-warning)]">
                ...
                {chatActive && (
                  <p className="shrink-0 border-t border-[var(--cf-border)] px-2.5 py-1.5 text-badge leading-snug text-[var(--cf-text-muted)]">
              ...
              <>
                <button
                  ...
                  className="flex shrink-0 items-center gap-1 border-b border-[var(--cf-border)] px-2 py-1.5 text-badge font-medium text-[var(--cf-text-muted)] hover:text-[var(--cf-text)]"
                >
                  <ChevronLeft size={12} />
                  {labelOf(browsing)}
                </button>
                <div className="min-h-0 flex-1 overflow-auto p-1">
                  {versions === undefined ? (
                    <p className="flex items-center gap-1.5 px-2 py-2 text-badge text-[var(--cf-text-muted)]">
                    ...
                    <>
                      ...
                      {versions.length === 0 && (
                        <p className="px-2 py-2 text-badge leading-snug text-[var(--cf-text-muted)]">
                          {t("chat.noModels")}
                        </p>
                      )}
                    </>
            ...
            <button
              ...
              className="flex shrink-0 items-center gap-1.5 border-t border-[var(--cf-border)] px-2.5 py-1.5 text-badge text-[var(--cf-text-muted)] hover:text-[var(--cf-accent)]"
            >
              <Settings2 size={11} />
...
function VersionItem({ label, selected, onClick }: { label: string; selected: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`flex w-full items-center gap-1.5 rounded-md px-2 py-1 text-left text-ui ${
        selected
          ? "bg-[var(--cf-accent-soft)] text-[var(--cf-accent)]"
