...
export interface VariableInputProps {
   ...
   * ordinary input. */
  variableContext?: VariableContext | null | undefined;
...
export function VariableInput({
  ...
  disabled = false,
  className = "",
  fieldClassName = "px-2 py-1.5 text-ui",
  ariaLabel,
  onPaste,
  onKeyDown,
}: VariableInputProps) {
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
    if (menuOpen) {
      ...
      if (e.key === "Enter" || e.key === "Tab") {
        ...
        if (first) accept(suggestions[activeIndex]?.name ?? first.name);
        return;
      }
      if (e.key === "Escape") {
        e.preventDefault();
        ...
        createPortal(
          ...
          >
            {suggestions.map((item, index) => (
              ...
              >
                <span className="shrink-0 font-mono text-badge text-[var(--cf-accent)]">{item.name}</span>
                <span className="min-w-0 flex-1 truncate font-mono text-badge text-[var(--cf-text-muted)]">
                  {item.detail}
                </span>
                <span className="shrink-0 text-badge uppercase tracking-wide text-[var(--cf-text-muted)]">
                  {item.badge}
                </span>
              </button>
            ))}
            <p className="px-1.5 pb-0.5 pt-1 text-badge text-[var(--cf-text-muted)]">{t("api.env.suggestHint")}</p>
          </div>,
          document.body,
        )}
            ...
            {names.map((name) => {
              ...
              if (editing === name) {
                return (
                  <div key={name} className="flex items-center gap-1.5 py-0.5">
                    <span className="shrink-0 font-mono text-badge text-[var(--cf-accent)]">{name}</span>
                    <input
                      autoFocus
                      value={draft}
                      ...
                      className="min-w-0 flex-1 rounded border border-[var(--cf-accent)] bg-transparent px-1.5 py-0.5 font-mono text-badge outline-none"
                    />
                    <IconButton
                      label="common.save"
                      icon={Check}
                    ...
                    />
                    {/* `X` is right here — this dismisses the inline editor rather than removing
                        anything (icon dictionary, §II.3). */}
                    <IconButton
                      label="common.cancel"
                      icon={X}
                      className="shrink-0"
              ...
              return (
                <div key={name} className="group flex items-baseline gap-2 py-0.5">
                  <span className="shrink-0 font-mono text-badge text-[var(--cf-accent)]">{name}</span>
                  <span
                    className="min-w-0 flex-1 truncate font-mono text-badge"
                  ...
                  {editable && (
                    <IconButton
                      label={found ? "api.env.editValue" : "api.env.defineValue"}
                      icon={Pencil}
