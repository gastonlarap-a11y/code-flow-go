...
export function openInGroups(
  ...
  const evicted = previewIndex >= 0 ? (target.paths[previewIndex] ?? null) : null;
...
export function closeGroupInGroups(
  ...
  // the eye is already nearest to. `remaining` is never empty: `groups.length >= 2` above and
  // exactly one group (`closing`) was filtered out.
