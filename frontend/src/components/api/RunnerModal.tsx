...
import { moveBy, moveTo } from "../../lib/ui/reorder";
...
import { getEffectiveAuthChain, getVariableContext } from "../../state/apiStore";
...
import { apiPickFile, apiReadTextFile, apiSaveFile } from "../../lib/ipc/apiCommands";
import type {
  ApiFolder,
  ApiProtocol,
  ApiRequestRow,
...
const RUNNER_VIEWS: readonly TabOption<"setup" | "results">[] = [
  { id: "setup", labelKey: "api.runner.setup" },
  { id: "results", labelKey: "api.runner.results" },
];
...
export function RunnerModal({
  ...
  const collectionName = useApiTreeStore((s) => s.collections.find((c) => c.id === collectionId)?.name ?? "",
  );
  ...
  const beginDrag = useCallback((e: React.PointerEvent<HTMLElement>, id: string) => {
    ...
    const onUp = (ev: PointerEvent) => {
      ...
      setDragId(null);
      setDropIndex(null);
      if (target !== null) setOrder((previous) => moveTo(previous, id, target));
    };
  ...
  const run = async () => {
    ...
    const { settings } = useApiSettingsStore.getState();
    const { cookies } = useApiCookieStore.getState();
    const { activeEnvironmentId } = useApiEnvironmentStore.getState();
    const seed = getVariableContext(collectionId);
    ...
    const execute = async (
      ...
      try {
        resolved = await resolveRequest(spec, ctx, getEffectiveAuthChain(row.id), settings, cookies);
      } catch (e) {
    ...
    if (config.persistVariables) {
      const after = useApiEnvironmentStore.getState();
      const afterTree = useApiTreeStore.getState();
      ...
      const collection = afterTree.collections.find((candidate) => candidate.id === collectionId);
      if (collection) {
        await afterTree.updateCollection({
          ...collection,
  ...
  return (
    <ApiModal
      ...
      toolbar={
        <Tabs
          options={RUNNER_VIEWS}
          activeId={view}
          onSelect={setView}
          layoutId="cf-runner-view"
          label={t("api.runner.title")}
        />
      }
      footer={
        <>
          <span className="mr-auto min-w-0 truncate text-badge text-[var(--cf-text-muted)]">
            {progress
          ...
          {report && !running && (
            <Button variant="ghost" size="sm" icon={Download} onClick={() => void exportReport()}>
    ...
    >
      {view === "setup" ? (
        <div {...tabPanelProps("cf-runner-view", view)} className="flex min-h-0 flex-1">
          ...
          <div className="flex min-w-0 flex-1 flex-col border-r border-[var(--cf-border)]">
            <div className="flex shrink-0 items-center gap-2 border-b border-[var(--cf-border)] px-3 py-1.5">
              ...
              <span className="text-badge text-[var(--cf-text)]">{t("api.runner.selectAll")}</span>
            ...
            <div className="min-h-0 flex-1 overflow-auto p-1">
              {order.length === 0 && (
                <p className="p-3 text-ui text-[var(--cf-text-muted)]">{t("api.noRequests")}</p>
              )}
              {order.map((id, index) => {
                ...
                return (
                  <div key={id} data-cf-runid={id}>
                    ...
                    >
                      {/* A real button, not a grab-only span: the arrow keys move the row the same
                          way a drag does, through `moveBy`, so the run order is reachable without a
                          pointer. `aria-label` carries the position, because "reorder" alone does
                          not tell a screen-reader user what they are about to move or from where. */}
                      <Tooltip label={t("api.runner.reorderHint")}>
                        <button
                          type="button"
                          aria-label={t("api.runner.reorderRow", { n: index + 1, total: order.length })}
                          onPointerDown={(e) => beginDrag(e, id)}
                          onKeyDown={(e) => {
                            const step = e.key === "ArrowUp" ? -1 : e.key === "ArrowDown" ? 1 : null;
                            if (step === null) return;
                            e.preventDefault();
                            setOrder((previous) => moveBy(previous, id, step));
                          }}
                          className="cf-focusable cf-interactive flex h-6 w-6 shrink-0 cursor-grab items-center justify-center rounded-md text-[var(--cf-text-muted)]"
                        >
                          <GripVertical size={14} aria-hidden />
                        </button>
                      </Tooltip>
                      <Checkbox checked={!excluded.has(id)} onChange={() => toggleOne(id)} />
                      <span className="w-[44px] shrink-0 font-mono text-badge font-semibold uppercase text-[var(--cf-accent)]">
                        {row.method}
                      </span>
                      <span className="min-w-0 flex-1 truncate text-ui text-[var(--cf-text)]">
                        {row.name}
                      </span>
                      <span className="min-w-0 max-w-[45%] truncate font-mono text-badge text-[var(--cf-text-muted)]">
                        {row.url}
                      </span>
                    </div>
          ...
          <div className="w-[240px] shrink-0 overflow-auto p-3">
            <label htmlFor={iterationsId} className="mb-1 block text-badge font-medium text-[var(--cf-text-muted)]">
            ...
            <label htmlFor={delayId} className="mb-1 mt-3 block text-badge font-medium text-[var(--cf-text-muted)]">
              {t("api.runner.delay")}
            </label>
            <Field
              id={delayId}
            ...
            <label className="mb-1 mt-3 block text-badge font-medium text-[var(--cf-text-muted)]">
              {t("api.runner.dataFile")}
            </label>
            {dataPath === null ? (
              <Button variant="ghost" size="sm" icon={FileSpreadsheet} onClick={() => void pickData()}>
              ...
              <div className="rounded-md border border-[var(--cf-border)] p-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <Tooltip label={dataPath}>
                    <span className="min-w-0 flex-1 truncate font-mono text-badge">
                      {dataPath.split(/[\\/]/).pop()}
                    </span>
                  </Tooltip>
                  <IconButton
                    label="api.runner.removeData"
                    icon={Trash2}
                    variant="danger"
                    onClick={() => {
                      setData([]);
            ...
            <label className="mt-4 flex cursor-pointer items-start gap-2">
              ...
              <span className="text-ui text-[var(--cf-text)]">
            ...
            <label className="mt-2 flex cursor-pointer items-start gap-2">
              ...
              <span className="text-ui text-[var(--cf-text)]">{t("api.runner.stopOnError")}</span>
            </label>
          </div>
        </div>
      ) : (
        <div {...tabPanelProps("cf-runner-view", view)} className="min-h-0 flex-1 overflow-auto p-3">
          {capped && (
            <p className="mb-2 flex items-center gap-1.5 rounded-md border border-[var(--cf-warning)] bg-[color-mix(in_oklab,var(--cf-warning)_12%,transparent)] px-2 py-1.5 text-badge text-[var(--cf-text)]">
          ...
          {!report ? (
            <p className="p-3 text-ui text-[var(--cf-text-muted)]">
              {running ? (
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2 px-2.5 py-1.5">
                      ...
                      <span className="w-[68px] shrink-0 text-badge uppercase tracking-wide text-[var(--cf-text-muted)]">
                      ...
                      <span className="w-[44px] shrink-0 text-right font-mono text-badge text-[var(--cf-text-muted)]">
                        {item.status ?? "—"}
                      </span>
                      <span className="w-[64px] shrink-0 text-right font-mono text-badge text-[var(--cf-text-muted)]">
                        {item.durationMs} ms
                      </span>
                      <span className="w-[76px] shrink-0 text-right text-badge text-[var(--cf-text-muted)]">
                        {item.tests.length > 0
                    ...
                    {item.error !== null && (
                      <p className="px-2.5 pb-1.5 pl-[38px] text-badge leading-snug text-[var(--cf-danger)]">
                        {item.error}
                      </p>
                    )}
                    ...
                    {failedTests.length > 0 && (
                      <ul className="px-2.5 pb-1.5 pl-[38px]">
                        {failedTests.map((test: TestResult, testIndex) => (
                          <li
                            key={testIndex}
                            className="text-badge leading-snug text-[var(--cf-text-muted)]"
                          >
