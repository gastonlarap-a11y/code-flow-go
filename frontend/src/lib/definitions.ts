...
export function findImportSource(text: string, symbol: string): string | null {
  ...
  for (const m of text.matchAll(es)) {
    // `spec` (the `from "..."` target) is a required group — always captured when `m` matched.
...
/**
 * A regex matching the *declaration* of `symbol`, in syntax the sidecar's regex engine accepts (the
...
export function moduleSpecifierAt(lineText: string, column: number): string | null {
  ...
  for (const match of lineText.matchAll(/['"]([^'"]+)['"]/g)) {
    // Group 1 is required by the pattern, so it's always captured when `match` exists.
    const text = match[1]!;
    const start = match.index + 1;
    const end = start + text.length;
    // `column` is 1-based and may sit just past the last character.
    if (column >= start && column <= end + 1) return text;
  }
  return null;
}
