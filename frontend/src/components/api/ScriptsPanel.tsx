    ...
    <div className="flex h-full min-h-0">
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex shrink-0 items-center gap-2 border-b border-[var(--cf-border)] px-3 py-1.5">
          ...
          <span className="text-badge text-[var(--cf-text)]">
            {t(kind === "pre" ? "api.scripts.preRequest" : "api.scripts.postResponse")}
          </span>
          <span className="truncate text-badge text-[var(--cf-text-muted)]">{t("api.scripts.docsHint")}</span>
        </div>
        ...
        <div className="shrink-0 px-2 py-1.5">
          ...
          <div className="flex items-center gap-1.5 rounded-md border border-[var(--cf-border)] bg-[var(--cf-bg)] px-1.5 py-1">
            ...
            <input
              ...
              className="min-w-0 flex-1 bg-transparent text-badge text-[var(--cf-text)] outline-none placeholder:text-[var(--cf-text-muted)]"
            />
          </div>
        </div>
        <div className="min-h-0 flex-1 overflow-auto px-1 pb-2">
          {snippets.map((snippet) => (
            <Tooltip key={snippet.label} label={t("api.scripts.insertSnippet")}>
              <button
                type="button"
                onClick={() => insert(snippet.code)}
                className="group flex w-full items-start gap-1.5 rounded px-1.5 py-1 text-left text-badge text-[var(--cf-text-muted)] hover:bg-[var(--cf-accent-soft)] hover:text-[var(--cf-accent)]"
              >
                {/* Decoration, not a control: the whole row is the button and it already reads as
                    one. Revealing it on hover only sharpens what is already there. */}
                <Plus size={11} aria-hidden className="mt-0.5 shrink-0 opacity-0 group-hover:opacity-100" />
