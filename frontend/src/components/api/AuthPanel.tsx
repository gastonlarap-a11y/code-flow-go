    ...
    <div className="flex items-center rounded-md border border-[var(--cf-border)] bg-[var(--cf-surface)] focus-within:border-[var(--cf-accent)]">
      {masked ? (
        <input
          ...
          className="min-w-0 flex-1 bg-transparent px-2 py-1.5 text-ui leading-5 text-[var(--cf-text)] outline-none placeholder:text-[var(--cf-text-muted)] disabled:cursor-not-allowed"
        />
      ) : (
        <VariableInput
          value={value}
...
function Row({ label, hint, children }: { label: string; hint?: string; children: ReactNode }) {
  return (
    <>
      <span className="pt-1.5 text-ui text-[var(--cf-text-muted)]">{label}</span>
      <div className="min-w-0">
        {children}
        {hint && <p className="mt-1 text-badge text-[var(--cf-text-muted)]">{hint}</p>}
      </div>
    </>
  );
}
...
function Note({ tone = "muted", children }: { tone?: "muted" | "warning"; children: ReactNode }) {
  ...
  return (
    <p className="flex items-start gap-1.5 text-badge" style={{ color }}>
...
export function AuthPanel({ tabId }: { tabId: string }) {
  const t = useT();
  const tab = useApiTabsStore((s) => s.openTabs.find((entry) => entry.id === tabId));
  ...
  const ctx = useMemo(
    // `getVariableContext` reads the environment and collection lists out of the store itself, so
    ...
    <div className="flex h-full min-h-0 flex-col gap-3 overflow-auto p-3">
      ...
      {auth.type === "inherit" ? (
        <div className="flex flex-col gap-2 rounded-lg border border-[var(--cf-border)] bg-[var(--cf-surface)] p-3">
          {source ? (
            <>
              <p className="flex items-center gap-1.5 text-ui text-[var(--cf-text)]">
  ...
  return (
    <div className="rounded-md border border-[var(--cf-border)] bg-[var(--cf-surface)] p-2">
      <div className="mb-1 flex items-center justify-between">
        <span className="text-badge font-medium text-[var(--cf-text-muted)]">{t("api.auth.jwtPreview")}</span>
        {"token" in result && result.token !== "" && (
          <RevealToggle revealed={revealed} onToggle={() => setRevealed((on) => !on)} />
        )}
      </div>
      {"error" in result ? (
        <Note tone="warning">{result.error}</Note>
      ) : (
        <p className="select-text break-all font-mono text-badge leading-[1.5] text-[var(--cf-text)]">
        ...
        <>
          <div className="flex flex-wrap items-center gap-2">
            <Button
              ...
              icon={ShieldCheck}
              pending={busy}
              disabled={!grant.supported && !hasRefreshToken}
