          ...
          <div key={tpl.key} className="rounded-lg border border-[var(--cf-border)]">
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="block truncate text-body font-medium text-[var(--cf-text)]">
                  {t(tpl.labelKey)}
                </span>
                <span className="block truncate text-badge text-[var(--cf-text-muted)]">{engineFor(tpl.task)}</span>
              </span>
              {savedFlash === tpl.key ? (
                <span className="shrink-0 text-badge font-medium text-[var(--cf-success)]">
                  {t("settings.saved")}
                </span>
              ) : isCustom ? (
                <span className="shrink-0 rounded-full bg-[var(--cf-accent-soft)] px-2 py-0.5 text-badge font-medium text-[var(--cf-accent)]">
                  {t("settings.templateCustom")}
                </span>
              ) : (
                <span className="shrink-0 rounded-full bg-black/[0.05] px-2 py-0.5 text-badge font-medium text-[var(--cf-text-muted)] dark:bg-white/[0.08]">
            ...
            {isOpen && (
              <div className="border-t border-[var(--cf-border)] p-3">
                <p className="mb-2 text-badge leading-snug text-[var(--cf-text-muted)]">{t(tpl.hintKey)}</p>
                <textarea
                ...
                <div className="mt-1.5 flex items-center justify-between">
                  ...
                  {isCustom && (
                    <Button variant="ghost" size="sm" icon={RotateCcw} onClick={() => void reset(tpl)}>
